...
public class DiskManager {
    ...
    public void WritePage(PageId p , ByteBuffer buff){
        ...
        try(RandomAccessFile file= new RandomAccessFile(path,"rw")){
            if(Files.exists(Paths.get(path))){
                file.seek((long) (p.getPageIdx() + 1) * dbConfiginstance.getPagesize());
                file.write(buff.array());
            }else{
                System.out.println("Le fichier n'existe pas");
                file.setLength(dbConfiginstance.getDm_maxfilesize());
    ...
    public void ReadPage(PageId p, ByteBuffer buff) {
        try {
            ...
            if (Files.exists(Paths.get(path))) {
                ...
                file.seek((long) p.getPageIdx() * dbConfiginstance.getPagesize());
                byte[] pageData = new byte[dbConfiginstance.getPagesize()]; // Création d'un tableau de bytes pour lire les données
